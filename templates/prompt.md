...
1. 遵循 Conventional Commits 规范（feature/bugfix/docs/style/refactor/test/chore 等）
...
8. 如果是以 json 格式输出请进行 minify

示例格式：
```
{% if json_output %}
{
  "subject": "[feature](用户登录): 添加用户登录功能",
  "body": " - 实现 JWT token 验证\n - 添加登录/登出接口\n - 更新用户状态管理"
}
{% else %}
