...
fn mask_token(s: &str) -> String {
    ...
    let suffix: String = chars[len - 8..].iter().collect();
...
struct Args {
    ...
    /// AI 接口的 api key
    #[clap(short, long, default_value_t = mask_token(&resolve_env(&vec!("ANTHROPIC_API_KEY", "ANTHROPIC_AUTH_TOKEN")).unwrap_or_default()))]
    api_key: String,
